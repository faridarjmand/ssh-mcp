...
| Managed SSH entries | `src/ssh/managed-hosts.ts` | Validate, back up, persist, and overlay dashboard-owned hosts |
...
| UI | `src/ui/App.tsx`, `src/ui/HostEditorModal.tsx`, `src/ui/fleet-refresh.ts`, `src/ui/TerminalModal.tsx`, `src/ui/AgentModal.tsx`, `src/ui/styles.css` | Inventory/editor, bounded fleet refresh, metrics, terminal, agent config, and index UI |
| Tests | `tests/*` | Parser, managed hosts, security boundary, index, MCP, and fleet-refresh behavior |
...
4. Keep source SSH config read-only; write only the dedicated managed file behind the token-required gate.
5. Keep automatic remote commands fixed and read-only.
6. Add or update a test for security-boundary changes.
7. Run `npm run check`.
8. Update this map and relevant docs when modules or behavior move.
