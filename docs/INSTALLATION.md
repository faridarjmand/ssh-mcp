...
## 6. Protect or expose the listener
...
Run `ssh your-alias` in a normal terminal and resolve host-key, key-permission, or authentication errors there. In Docker, `${HOME}/.ssh` is read-only, so add host keys before starting the container.
